...
# Non-TNEF can use content-type for attachment directly but TNEF is content-type
# application/ms-tnef and we use the extension to decide.  The non-TNEF version
# sets the appropriate constant below.
...
class EmailExtractor(object):
    ...
    """
    email_select_line = re.compile(''.join((
        '\A',
        '(?:',
        '(?:', # whitespace line
        '\s*',
            ...
            try:
                ...
                tkinter.messagebox.showinfo(
                    ...
                    message=''.join((
                        ...
                        '\n\nfailed.\n\nHopefully because the directory does ',
                        'not exist yet: it could have been deleted.')))
                return
        return None, additional
...
class ExtractEmail(object):
    ...
    def __init__(
        ...
        **soak):
        ...
        if collect_conf:
            try:
                cc = open(os.path.join(eventdirectory, collect_conf),
                          encoding='utf8')
                try:
        ...
        if self.mostrecentdate is not None:
            try:
                ...
                tkinter.messagebox.showinfo(
                    parent=self.parent,
                    title='Get Emails',
                    message=''.join(('Most recent date format error.\n\n',
                                     'Please fix configuration file.')))
                return emails
        try:
            ms = self.mailstore
    ...
    def _get_emails_for_from_addressees(self):
        ...
        """
        return [e for e in self.get_emails()
...
class ExtractText(object):
    ...
    def message(self):
        ...
        if self._message is None:
            ...
            try:
                self._message = message_from_binary_file(mf, _class=MessageFile)
                self._date = [parsedate_tz(d)[:-1]
                              for d in self.message.get_all('date', [])]
                self._delivery_date = [parsedate_tz(d)[:-1]
                                       for d in self.message.get_all(
                                           'delivery-date', [])]
            finally:
                mf.close()
    ...
    def encoded_text(self):
        ...
        if self._encoded_text is None:
            ...
            # This may make the one character adjustment done by _insert_entry()
            # in the SourceEdit class redundant.
            # The reason to not do this all along was to avoid making any change
            # at all between the selected email payload and the original version
            # held in the difference file, including an extra newline.  Attempts
            # to wrap difflib functions to cope seem not worth it, if such prove
            # possible at all.
        ...
        if self._extracted_text is None:
            ...
            for p in self.message.walk():
                ...
                if ct in ems.pdf_content_type:
                    ...
                    if not tnefparse:
                        text.append(
                            ''.join(('Cannot process attachment: ',
                    ...
                    # As far as I know, the only application/ms-tnef attachments
                    # ever received by me at time of writing contain just *.txt
                    # attachments. These are not processed by this route.
    ...
    def _get_ss_text_using_xlsx2csv(self, filename, payload, text):
        ...
        xlsx2csv.Xlsx2csv(taf,
                          skip_empty_lines=True,
                          sheetid=0,
                          dateformat='%Y-%m-%d',
                          outputencoding='utf-8',
                          ).convert(os.path.join(ems.eventdirectory,
                                                 'xls-attachments'),
                                    sheetid=0)
    ...
    def _get_pdf_text_using_xpdf(self, filename, payload, text):
        ...
        try:
            ...
            pass
        op = open(
            os.path.join(dirbase, 'pdf-attachments', a), 'wb')
        try:
            op.write(payload)
        finally:
            op.close()
    ...
    def get_pdf_text_using_pdfminer3k(self,
                                      filename,
                                      payload,
                                      text,
                                      char_margin=150,
        ...
        **k captures other arguments which override defaults for pdfminer3k's
        ...
        laparams = layout.LAParams(char_margin=char_margin,
    ...
    def _read_file(self, csvpath):
        ...
        for c in 'utf-8', 'iso-8859-1':
            ...
            csvfile = open(
                csvpath, encoding='ascii', errors='replace')
            try:
                return io.StringIO(csvfile.read())
            except UnicodeDecodeError:
                pass
